...
void my_formula() {
	...
	//	∧ a3 := store(a2, 1+ i0, 1)
	c[3] = Z3_mk_eq(ctx, a3, Z3_mk_store(ctx, a2, Z3_mk_extract(ctx,0,0,Z3_mk_bvadd(ctx, i0, one)), one));
	...
	c[6] = Z3_mk_eq(ctx, p3, mk_binary_app(ctx, mk_pointer_sort_decl, mk_unary_app(ctx, get_object, addr_a) ,mem_8));
